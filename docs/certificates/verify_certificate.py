...
def main():
    ...
    print("Ì¥ç AINEXUS Certificate Verification")
    print("=" * 50)
    
    if result['valid']:
        print("‚úÖ CERTIFICATE VALID")
        print(f"Ì≥ã Certificate ID: {result['certificate_id']}")
        print(f"Ì±§ Issued To: {result['issued_to']}")
        print(f"ÔøΩÔøΩ Platform Version: {result['platform_version']}")
        print(f"‚è∞ Verified At: {result['verification_time']}")
    else:
        print("‚ùå CERTIFICATE INVALID")
        print(f"Ì≥ã Certificate ID: {result.get('certificate_id', 'Unknown')}")
        print(f"Ì∫´ Reason: {result['reason']}")
