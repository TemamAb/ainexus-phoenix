...
// Core Feature Definitions with proper TypeScript types
interface BotTier {
  name: string;
  count: number;
  role: string;
...
interface CoreFeature {
  ...
  protocols?: string[];
  models?: string[];
  tiers?: BotTier[]; // ADD THIS LINE - FIXES THE ERROR
}

const CORE_FEATURES: { [key: string]: CoreFeature } = {
  ...
  FLASH_LOAN_SYSTEM: {
    ...
    capabilities: ['Cross-Protocol Loan Aggregation', 'Atomic Arbitrage Execution'],
  ...
  AI_OPTIMIZATION_ENGINE: {
    ...
    capabilities: ['Real-Time Parameter Adjustment', 'Multi-Chain Opportunity Scoring'],
  ...
  THREE_TIER_BOT_SYSTEM: {
    ...
    capabilities: ['Opportunity Detection', 'Trade Execution', 'Risk Monitoring'],
    tiers: [ // THIS NOW EXISTS IN THE TYPE DEFINITION
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (deploymentMode === 'sim' && isSimulating) {
        ...
        const tradeProfit = (Math.random() * 50 - 10);
        ...
        const tradeProfit = (Math.random() * 100 - 20);
            ...
            <div className="status-left">
              ...
              {isBlockchainConnected && (
                <div className="blockchain-info">
                  Ì¥ó {connectedNetworks.length} Networks ‚Ä¢ Block: {blockHeight.toLocaleString()}
            ...
            <div className="features-grid">
              {Object.entries(CORE_FEATURES).map(([key, feature]) => (
                <div key={feature.id} className={`feature-card ${activatedFeatures[feature.id] ? 'active' : 'inactive'}`}>
                  ...
                  {/* FIXED: tiers now exists in the type definition */}
                  {feature.id === 'three-tier-bots' && feature.tiers && activatedFeatures[feature.id] && (
